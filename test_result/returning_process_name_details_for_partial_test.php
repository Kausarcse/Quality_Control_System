...
$obj->connection();
	...
	$version_and_process= mysqli_real_escape_string($con, $_POST['version_and_process']);
    ...
    $process_serial_no = ( (int) $splitted_data[3] - 1);
	...
	$option = "";
   	...
   	$sql_for_porcess_selection="SELECT DISTINCT apv.process_serial_no,apv.process_name,apv.process_id,ptti.version_number,ptti.pp_number,
   								ptti.design,ptti.style,ptti.finish_width_in_inch, ptti.after_trolley_number_or_batcher_number from partial_test_for_test_result_info ptti
	...
	while( $row_for_pp_version = mysqli_fetch_array($result_for_porcess_selection))
	{
		...
		$data_for_all_process_serial_no[] = $row_for_pp_version['process_serial_no'];
		...
		$after_trolley_number_or_batcher_number=$row_for_pp_version['after_trolley_number_or_batcher_number'];

	}  
		//print_r($data_for_all_process_id);
		...
		// echo $sql = "SELECT ptfti.process_name
		...
		// 		and  ptfti.process_serial_no = $process_serial_no
		// 		and  ptfti.finish_width_in_inch = '$finish_width_in_inch' order by ptfti.partial_test_for_test_result_id DESC LIMIT 1";


		$sql = "SELECT ptfti.process_name
		         ,ptfti.after_trolley_number_or_batcher_number
		          FROM partial_test_for_test_result_info ptfti
				WHERE ptfti.pp_number='$pp_number' AND ptfti.version_number='$version_number'
				and  ptfti.style = '$style'
				and  ptfti.design = '$design'
				and  ptfti.process_serial_no = $process_serial_no
