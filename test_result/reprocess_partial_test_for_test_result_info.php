...
<script type='text/javascript' src='test_result/reprocess_partial_test_for_test_result_info_form_validation.js'></script>
...
function get_qc_standard_additional_info(pp_number)
{
 ...
 function returning_value_for_shift(alternate_partial_test_for_test_result_creation_date_time)
{
    splitted_time = alternate_partial_test_for_test_result_creation_date_time.split(':');
...
{  
    ...
    $.ajax({
            url: 'test_result/returning_re_process_name.php',
            dataType: 'text',
            type: 'post',
            contentType: 'application/x-www-form-urlencoded',
            data: version_id ,
            ...
            success: function( data, textStatus, jQxhr )
            {       
                document.getElementById('process_name_to').innerHTML=data;
                    
                /*         FOR REPROCESS            */  

                $.ajax({
                        url: 'test_result/returning_all_for_process_name_for_reprocess_form.php',
                        dataType: 'text',
                        type: 'post',
                        contentType: 'application/x-www-form-urlencoded',
                        data: version_id ,
                        ...
                        success: function( data, textStatus, jQxhr )
                        {       
                            document.getElementById('process_name_from').innerHTML=data;  
                        },
                        error: function( jqXhr, textStatus, errorThrown )
                        {       
...
function return_machine_name_process_from(process_name)
{ 
    ...
    {
        document.getElementById('form-group_for_before_trolley_number_or_batcher_number').style.display = 'none';
        document.getElementById('before_trolley_number_or_batcher_number').innerHTML = '';
        document.getElementById('before_trolley_number_or_batcher_number').value = '';
        document.getElementById('form-group_for_after_trolley_number_or_batcher_number').style.display = 'none';
        document.getElementById('after_trolley_number_or_batcher_number').innerHTML = '';
    ...
    else if((process_id == 'proc_1') || (process_id == 'proc_21') || (process_id == 'proc_22'))
    {
        $.ajax({
                url: 'test_result/returning_value_for_first_after_trolley_or_batcher_quantity.php',
                dataType: 'text',
                type: 'post',
                contentType: 'application/x-www-form-urlencoded',
                data: value_for_trolley_quantity_data,
                        
                success: function( data, textStatus, jQxhr )
                {       
                    var splitted_qty_data = data.split('?fs?');
                    var after_trolley_or_batcher_qty = splitted_qty_data[1];
                    var after_trolley_or_batcher_out_time = splitted_qty_data[2];
                    document.getElementById('before_trolley_number_or_batcher_number').innerHTML = '';                    
    ...
    {
        $.ajax({
                url: 'test_result/returning_process_name_details_for_reprocess.php',
                dataType: 'text',
                type: 'post',
                contentType: 'application/x-www-form-urlencoded',
...
function return_trolley_quantity(before_trolley_number_or_batcher_number)
{
    ...
    var process_name=document.getElementById('process_name_to').value;
...
function returning_get_all_data_field_for_trf(trf_id)
{
    
    if(trf_id == 'select')
    {
        $('#div_returning_all_data_field_for_trf').hide();
        $('#div_all_data_without_trf').show();
    }
    else
    {
        $('#div_all_data_without_trf').hide();
        $('#div_returning_all_data_field_for_trf').show();
    }
    
    var url_encoded_form_data = 'trf_id='+trf_id;
       
       $.ajax({
        url: 'test_result/returning_get_all_data_field_required_for_trf.php',
        dataType: 'text',
        type: 'post',
        contentType: 'application/x-www-form-urlencoded',
        ...
        {
            // alert(data);
            document.getElementById('div_returning_all_data_field_for_trf').innerHTML = data;
        },
        error: function( jqXhr, textStatus, errorThrown )
        {
            //console.log( errorThrown );
...
function sending_data_of_partial_test_for_test_result_form_for_saving_in_database()
{
    ...
    if(validate != false)
    {
        ...
        $.ajax({
            url: 'test_result/reprocess_partial_test_for_test_result_info_saving.php',
            dataType: 'text',
            type: 'post',
            contentType: 'application/x-www-form-urlencoded',
            data: url_encoded_form_data,
                ...
                { 
                    ...
                    $.ajax({
                            ...
                            { 
                                ...
                                if(process_name== "Re-Greige Receiving" || process_name == '2nd-Re-Greige Receiving' || process_name == '3rd-Re-Greige Receiving' || process_name == '4th-Re-Greige Receiving')
                                {
                                ...
                                else if(process_name== "Re-Singeing" || process_name== "2nd-Re-Singeing" || process_name== "3rd-Re-Singeing" || process_name== "4th-Re-Singeing")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_singeing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Desizing" || process_name== "2nd-Re-Desizing" || process_name== "3rd-Re-Desizing" || process_name== "4th-Re-Desizing")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_desizing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                } 
                                else if(process_name== "Re-Singeing & Desizing" || process_name== "2nd-Re-Singeing & Desizing" || process_name== "3rd-Re-Singeing & Desizing" || process_name== "4th-Re-Singeing & Desizing")
                                {
                                ...
                                else if(process_name== "Re-Scouring" || process_name== "2nd-Re-Scouring" || process_name== "3rd-Re-Scouring" || process_name== "4th-Re-Scouring")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_scouring_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Bleaching" || process_name== "2nd-Re-Bleaching" || process_name== "3rd-Re-Bleaching" || process_name== "4th-Re-Bleaching")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_bleaching_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Scouring & Bleaching" || process_name== "2nd-Re-Scouring & Bleaching" || process_name== "3rd-Re-Scouring & Bleaching" || process_name== "4th-Re-Scouring & Bleaching")
                                {
                                ...
                                else if(process_name== "Re-Ready For Mercerize" || process_name== "2nd-Re-Ready For Mercerize" || process_name== "3rd-Re-Ready For Mercerize" || process_name== "4th-Re-Ready For Mercerize")
                                {
                                ...
                                else if(process_name== "Re-Mercerize" || process_name== "2nd-Re-Mercerize" || process_name== "3rd-Re-Mercerize" || process_name== "4th-Re-Mercerize")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_mercerize_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Ready For Print" || process_name== "2nd-Re-Ready For Print" || process_name== "3rd-Re-Ready For Print" || process_name== "4th-Re-Ready For Print")
                                {
                                ...
                                else if(process_name== "Re-Printing" || process_name== "2nd-Re-Printing" || process_name== "3rd-Re-Printing" || process_name== "4th-Re-Printing")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_printing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Curing" || process_name== "2nd-Re-Curing" || process_name== "3rd-Re-Curing" || process_name== "4th-Re-Curing")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_curing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Ready For Dyeing" || process_name== "2nd-Re-Ready For Dyeing" || process_name== "3rd-Re-Ready For Dyeing" || process_name== "4th-Re-Ready For Dyeing")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_ready_for_dying_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Washing" || process_name== "2nd-Re-Washing" || process_name== "3rd-Re-Washing" || process_name== "4th-Re-Washing")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_washing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Ready For Raising" || process_name== "2nd-Re-Ready For Raising" || process_name== "3rd-Re-Ready For Raising" || process_name== "4th-Re-Ready For Raising")
                                {
                                ...
                                else if(process_name== "Re-Raising" || process_name== "2nd-Re-Raising" || process_name== "3rd-Re-Raising" || process_name== "4th-Re-Raising")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_raising_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Finishing" || process_name== "2nd-Re-Finishing" || process_name== "3rd-Re-Finishing" || process_name== "4th-Re-Finishing")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_finishing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Calander" || process_name== "2nd-Re-Calander" || process_name== "3rd-Re-Calander" || process_name== "4th-Re-Calander")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_calendering_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                } 
                                else if(process_name== "Re-Sanforizing" || process_name== "2nd-Re-Sanforizing" || process_name== "3rd-Re-Sanforizing" || process_name== "4th-Re-Sanforizing")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_sanforizing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }  
                                else if(process_name== "Re-Dyeing-Finish" || process_name== "2nd-Re-Dyeing-Finish" || process_name== "3rd-Re-Dyeing-Finish" || process_name== "4th-Re-Dyeing-Finish")
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_dyeing_finish_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }  
                                else if(process_name== "Re-Steaming" || process_name== "2nd-Re-Steaming" || process_name== "3rd-Re-Steaming" || process_name== "4th-Re-Steaming")
                                {
                                    // $('#div_full_form').load("process_qc_result/qc_result_for_steaming_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
                                else if(process_name== "Re-Dyeing" || process_name== "2nd-Re-Dyeing" || process_name== "3rd-Re-Dyeing" || process_name== "4th-Re-Dyeing")
                                {
                                    // $('#div_full_form').load("process_qc_result/qc_result_for_dying_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
 ...
 function sending_data_for_edit_form_for_getting_data_from_database(value)
 {
                ...
                { 
                    ...
                    else if(process_name== "Re-Singeing" || process_name== "2nd-Re-Singeing" || process_name== "3rd-Re-Singeing" || process_name== "4th-Re-Singeing")
                    {
                        ...
                        $('#div_full_form').load("process_qc_result/edit_qc_result_for_singeing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                    
                    }
                    else if(process_name== "Re-Desizing" || process_name== "2nd-Re-Desizing" || process_name== "3rd-Re-Desizing" || process_name== "4th-Re-Desizing")
                    {

                        $('#div_full_form').load("process_qc_result/edit_qc_result_for_desizing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                    ...
                    {
                        $('#div_full_form').load("process_qc_result/edit_qc_result_for_dyeing_finish_process.php?trf_data=" + encodeURIComponent(get_all_value));
                    }   
...
function sending_data_for_test_result_edit(partial_test_for_test_result_id)
{
    ...
    /*alert(partial_test_for_test_result_id);*/
    $('#div_full_form').load("test_result/edit_partial_test_for_test_result_info.php?partial_test_for_test_result_id="+encodeURIComponent(partial_test_for_test_result_id));
    $('#div_table').hide();
}
        ...
        <div class="panel panel-default" id="div_full_form">
            ...
            <form id='partial_test_for_test_result_form' name='partial_test_for_test_result_form'  action="" method="post" enctype="multipart/form-data" class="form-horizontal form-label-left" style="margin-bottom: 20px;">
                ...
                <div class="form-group form-group-sm" id="form-group_for_partial_test_for_test_result_creation_date">
                    ...
                    <div class="col-sm-2">
                        <input type="time" class="form-control" id="alternate_partial_test_for_test_result_creation_date_time" name="alternate_partial_test_for_test_result_creation_date_time"   placeholder="Hour:Minute" onchange="returning_value_for_shift(this.value)" >
                    </div>
                    <div class="col-sm-1">
            ...
            <div class="form-group form-group-sm" id="form-group_for_trf_no">
                ...
                <div class="col-sm-5">
                    <select  class="form-control " id="trf_id" name="trf_id"  onchange="returning_get_all_data_field_for_trf(this.value)">  <!--get_all_value_for_trf -->
                        <option select="selected" value="select">Select TRF No</option>
                        <?php 
                            $sql = 'select DISTINCT trf_id from `partial_test_for_trf_info` order by partial_test_for_trf_id desc';
                            $result= mysqli_query($con,$sql) or die(mysqli_error($con));
                            while( $row = mysqli_fetch_array( $result))
                            {
                                $trf_id=$row['trf_id'];
            ...
            </div>  <!-- End of <div class="form-group form-group-sm" id="form-group_for_trf_id"> -->

            <div id="div_returning_all_data_field_for_trf">
            ...
            <div class="form-group form-group-sm" id="form-group_for_process_name">
                <label class="control-label col-sm-3" for="process_name_from" style="margin-right:0px; color:#00008B;">Process Name From: <span style="color:red">*</span> </label>
                <div class="col-sm-5">
                    <select  class="form-control" id="process_name_from" name="process_name_from" >
            ...
            <div class="form-group form-group-sm" id="form-group_for_process_name">
                <label class="control-label col-sm-3" for="process_name_to" style="margin-right:0px; color:#00008B;">Process Name To: <span style="color:red">*</span> </label>
                <div class="col-sm-5">
                    <select  class="form-control" id="process_name_to" name="process_name_to" onchange="return_machine_name_process_from(this.value)">
                        <option select="selected" value="select">Select Re-Process Name</option>
                        <?php 
                                // $sql = 'select * from `process_name` order by row_id ASC';
                                // $result= mysqli_query($con,$sql) or die(mysqli_error($con));
                                // while( $row = mysqli_fetch_array( $result))
                    ...
                    <script>
                        $("#process_name_to").select2({
                            placeholder: "Select Re-Process Name",
                            theme: "classic",
                            selectOnClose: true,
                            closeOnSelect: true,
                            allowClear: true
           ...
           <div class="form-group form-group-sm" id="form-group_for_before_trolley_number_or_batcher_number">
            ...
            <div class="form-group form-group-sm" id="form-group_for_before_trolley_or_batcher_in_time">
                ...
                <div class="col-sm-5">
                    <input type="time" class="form-control" id="before_trolley_or_batcher_in_time" name="before_trolley_or_batcher_in_time" placeholder="Enter Before trolley number or batcher In Time" readonly>
                </div>
                   ...
                   <label class="control-label col-sm-3" for="care_label" style="color:#00008B;">Care Label:  </label>
						...
						<div class="form-group">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                              ...
                              <button type="button" name="submit" id="submit" class="btn btn-primary" onClick="sending_data_of_partial_test_for_test_result_form_for_saving_in_database()">Submit</button>
...
<table id="datatable-buttons" class="table table-hover table-bordered">
 <thead>
       <tr>
       <th>Test Result Creation Date</th>
       <th>TRF ID</th>
  ...
  <?php 
                  ...
                  $row_for_pp_color=mysqli_fetch_array($result_for_pp_color);
                     ...
                     if(($data_for_process_name == 'Re' || $data_for_process_name == '2nd' || $data_for_process_name == '3rd' || $data_for_process_name == '4th') && $process_name != '')
                     {
                      $date=date_create($row['partial_test_for_test_result_creation_date']);
                      $Test_result_creation_date = date_format($date,"d/m/Y");

   ?>

   <tr>
   <td><?php echo $Test_result_creation_date; ?></td>
      <td><?php 
  ...
  $data_for_delete = $row['partial_test_for_test_result_id'].'?fs?'.$row['process_id'].'?fs?'.$row['pp_number'].'?fs?'.$row['version_number'].'?fs?'.$row['finish_width_in_inch'];
  ...
  <button type="submit" id="" name=""  class="btn btn-danger btn-xs" onclick="sending_data_for_delete('<?php echo $data_for_delete; ?>')"> Delete</button>
       </td>
      <?php
