...
function get_refer_info(process_name)
{
    var pp_number = document.getElementById('pp_number').value;
    var value_for_data= 'pp_number_value='+pp_number;

    var splitdata = process_name.split("?fs?");
    var process_name = splitdata[1];
    ...
    {
        $.ajax({
            url: 'test_result/returning_refer_info_details.php',
            dataType: 'text',
            type: 'post',
            contentType: 'application/x-www-form-urlencoded',
            data: value_for_data,
            ...
            {  
                // alert(data);    
                document.getElementById('form_group_for_refer_info').style.display = 'block'; 
                document.getElementById('refer_report').innerHTML=data;
            },
            error: function( jqXhr, textStatus, errorThrown )
            {       
                alert(errorThrown);
    ...
    {
        document.getElementById('form_group_for_refer_info').style.display = 'none';
        //document.getElementById("get_refer_info").style.visibility = "hidden";
    }
...
{  
    ...
    $.ajax({
            url: 'test_result/returning_all_for_process_name.php',
            dataType: 'text',
            type: 'post',
            contentType: 'application/x-www-form-urlencoded',
            data: version_id ,
...
{ 
    ...
    // alert(process_id);
    var value_for_trolley_data= 'version_and_process='+version_id+'?fs?'+encodeURIComponent(process_name)+'?fs?';
...
function return_trolley_quantity(before_trolley_number_or_batcher_number)
{
    ...
    var process_name= encodeURIComponent(document.getElementById('process_name').value);
...
function sending_data_of_partial_test_for_test_result_form_for_saving_in_database()
{
    ...
    var refer_report = document.getElementById('refer_report').value;
    if(trf_id == 'select')
    {
    ...
    {
        $.ajax({
            url: 'test_result/partial_test_for_test_result_info_saving.php',
            dataType: 'text',
            type: 'post',
            contentType: 'application/x-www-form-urlencoded',
            data: url_encoded_form_data,
            success: function( data, textStatus, jQxhr )
            {
                ...
                { 
                    ...
                    $.ajax({
                            url: 'test_result/returning_value_for_test_result_for_partial.php',
                            dataType: 'text',
                            type: 'post',
                            contentType: 'application/x-www-form-urlencoded',
                            data: url_encoded_form_data,
                            ...
                            { 
                                ...
                                var get_all_value = data_split_for_result[0]+"?fs?partial_test_for_test_result_info?fs?partial_test_for_test_result_id?fs?"+refer_report;
                                ...
                                {
                                    $('#div_full_form').load("process_qc_result/qc_result_for_dyeing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                                }
...
{  
    ...
    $.ajax({
            ...
            { 
                ...
                {
                    // alert("There is no test result in this process.");
                    $('#div_full_form').load("process_qc_result/qc_result_for_steaming_process.php?trf_data=" + encodeURIComponent(get_all_value));
                }
                else if(process_name== "Ready For Dyeing")
                {
...
}//End of function sending_data_of_partial_test_for_test_result_form_for_getting_data_from_database()


    ////////////////////////////////////////     EDIT  TEST RESULT ENTRY Form   ////////////////////////////////////////////
...
function sending_data_for_edit_form_for_getting_data_from_database(value)
{
    ...
    $.ajax({
            ...
            { 
                ...
                {
                    // alert("There is no test result in this process.");
                    $('#div_full_form').load("process_qc_result/edit_qc_result_for_dyeing_process.php?trf_data=" + encodeURIComponent(get_all_value));
                }
                else if(process_name== "Washing")
                {
...
function sending_data_for_test_report(get_all_data)
{
    ...
    {
        $('#div_full_form').load("report/pass_fail_report_for_all_test.php?all_data="+encodeURIComponent(get_all_data));
    }
    else if (process_id == 'proc_10' || process_id == 'proc_12')
    {
        alert("There is no test report in this process.");
    }
    else
    {
    ...
    <div class="col-sm-12 col-md-12 col-lg-12">
        <div class="panel panel-default" id="div_full_form">
            ...
            <div class="form-group form-group-sm" id="form-group_for_process_name">
                ...
                <div class="col-sm-5">
                    <select  class="form-control " id="process_name" name="process_name" onchange="return_machine_name(this.value), get_refer_info(this.value)">
           ...
           <div class="form-group form-group-sm" id="form-group_for_before_trolley_number_or_batcher_number">
            ...
            <div class="form-group form-group-sm" id="form-group_for_machine_name">
                ...
                <div class="form-group form-group-sm" id="form_group_for_refer_info">
                    <label class="control-label col-sm-3" for="refer_report" style="margin-right:0px; color:#00008B;">Refer Report:  </label>
                    <div class="col-sm-5">
                        <select  class="form-control" id="refer_report" name="refer_report" >
                            <option value="select" select="selected">Select Refer Report</option>
                            <?php 
                                // $sql = 'select DISTINCT machine_name from `machine_name`';
                                // $result= mysqli_query($con,$sql) or die(mysqli_error($con));
                                // while( $row = mysqli_fetch_array( $result))
                        ...
                        <script>
                            $("#refer_report").select2({
                                placeholder: "Select Refer Report",
                                theme: "classic",
                                selectOnClose: true,
                                closeOnSelect: true,
                                allowClear: true
                ...
                </div> <!-- End of <div class="form-group form-group-sm" id="form-group_for_refer_report"> -->
            ...
            <table id="datatable-buttons" class="table table-hover table-bordered">
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($res_for_trf_for_partial_test)) 
                    {
                        ...
                        if(($data_for_at=='PT' || $data_for_at=='' || $data_for_at=='select' ) && $data_for_process_name != 'Re' && $data_for_process_name != '2nd' && $data_for_process_name != '3rd' && $data_for_process_name != '4th' && $process_name != '')
                        {
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    if($row['trf_id']=='select')
                                    {
                                        $get_all_data= "?fs?".$row['version_id'].'?fs?'.$row['pp_number']."?fs?".$row['process_id']."?fs?".$row['process_name']."?fs?".$row['style']."?fs?".$row['finish_width_in_inch']."?fs?".$row['before_trolley_number_or_batcher_number']."?fs?".$row['after_trolley_number_or_batcher_number']."?fs?".$row['after_trolley_or_batcher_qty'];
                                    }
                                    else
                                    {
                                        $get_all_data= $row['trf_id']."?fs?".$row['version_id'].'?fs?'.$row['pp_number']."?fs?".$row['process_id']."?fs?".$row['process_name']."?fs?".$row['style']."?fs?".$row['finish_width_in_inch']."?fs?".$row['before_trolley_number_or_batcher_number']."?fs?".$row['after_trolley_number_or_batcher_number'];
                                    }

                                    $data_for_delete = $row['partial_test_for_test_result_id'].'?fs?'.$row['process_id'].'?fs?'.$row['pp_number'].'?fs?'.$row['version_number'].'?fs?'.$row['finish_width_in_inch'].'?fs?'.$row['version_id']."?fs?".$row['before_trolley_number_or_batcher_number']."?fs?".$row['after_trolley_number_or_batcher_number']."?fs?".$row['after_trolley_or_batcher_qty'];
