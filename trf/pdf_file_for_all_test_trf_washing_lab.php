...
$pdf->Cell(95,4,"Test Report Type :  All Test", "1", "1","C",true);
...
$pdf->Cell(95,4,"Required Test (Washing Lab)", "1", "0","C",true);
$pdf->Ln(5);
...
//                             WHERE das.pp_number='$pp_number'  and das.customer_id='$customer_id'  and das.process_id='$process_id' and das.version_id='$version_id'";
							...
							{
								if($row_for_test['criteria_or_testing_lab']=='Washing Lab')
                                {
                                    ...
                                    if (in_array($row_for_test['id'], ['2','64']))
                                    {
                                        if($row_for_defining_process['washing_cycle_for_warp_for_washing_before_iron']<>'')
                                        {
                                            $washing_cycle_for_dimensional_stability = ' (Before '.$row_for_defining_process['washing_cycle_for_warp_for_washing_before_iron'].' wash)';
                                        }
                                        else if($row_for_defining_process['washing_cycle_for_weft_for_washing_before_iron']<>'')
                                        {
                                            $washing_cycle_for_dimensional_stability = ' (Before '.$row_for_defining_process['washing_cycle_for_weft_for_washing_before_iron'].' wash)';
                                        }
                                        else if($row_for_defining_process['washing_cycle_for_warp_for_washing_after_iron']<>'')
                                        {
                                            $washing_cycle_for_dimensional_stability = ' (After '.$row_for_defining_process['washing_cycle_for_warp_for_washing_after_iron'].' wash)';
                                        }
                                        elseif($row_for_defining_process['washing_cycle_for_weft_for_washing_after_iron']<>'')
                                        {
                                            $washing_cycle_for_dimensional_stability = ' (After '.$row_for_defining_process['washing_cycle_for_weft_for_washing_after_iron'].' wash)';

                                        }
                                       
                                        $uom_of_dimensional_stability = '%';
                                        ...
                                        $pdf->SetXY($x + 30, $y);
                                        $pdf->SetFont('Times','',6);
                                        $data = $row_for_test['test_method_name'].$washing_cycle_for_dimensional_stability;
                                        ...
                                        $pdf->SetFont('Times','',6.5);
                                        $b = 4.8;
                                        $pdf->MultiCell(11,$b,$uom_of_dimensional_stability,'1','L',TRUE);
                                        $pdf->SetXY($x + 86, $y);
                                        $pdf->MultiCell(9,$b,'','1','L',TRUE);
                                    }

                                    ///////////////   Appearance After Wash   ////////////////////////
                                    if (in_array($row_for_test['id'], ['3']))
                                    {
                                        if($row_for_defining_process['appearance_after_washing_cycle_fabric_wash']<>'')
                                        {
                                            $appearance_after_washing_cycle = $row_for_defining_process['appearance_after_washing_cycle_fabric_wash'];
                                        }
                                        else if($row_for_defining_process['appearance_after_washing_cycle_garments_wash']<>'')
                                        {
                                            $appearance_after_washing_cycle = $row_for_defining_process['appearance_after_washing_cycle_garments_wash'];
                                        }
                                        ...
                                        $pdf->SetXY($x + 30, $y);
                                        $pdf->SetFont('Times','',6);
                                        $data = $row_for_test['test_method_name'].' ('.$appearance_after_washing_cycle.')';
                                    ...
                                    if (in_array($row_for_test['id'], ['15']))
                                    {
                                     ...
                                     if (in_array($row_for_test['id'], ['17','61']))
                                     {
                                      ...
                                      ///////////////   Color Fastness to Perspiration Alkali     ////////////////////////
                                      if (in_array($row_for_test['id'], ['18','62']))
                                      {
                                    ...
                                    if (in_array($row_for_test['id'], ['19','63']))
                                    {
                                      ...
                                      if (in_array($row_for_test['id'], ['20','65']))
                                      {
                                       ...
                                       ///////////////   Migration of Color into PVC    ////////////////////////
                                    if (in_array($row_for_test['id'], ['26','70']))
                                    {
                                       ...
                                       if (in_array($row_for_test['id'], ['27']))
                                       {
                                        ...
                                        ///////////////   Cross Staining     ////////////////////////
                                        if (in_array($row_for_test['id'], ['31']))
                                        {
                                        ...
                                        if (in_array($row_for_test['id'], ['37']))
                                            {
                                                if($row_for_defining_process['smoothness_appearance_tolerance_washing_cycle']<>'')
                                                {
                                                    $smoothness_appearance_tolerance_washing_cycle = $row_for_defining_process['smoothness_appearance_tolerance_washing_cycle'];
                                                }
                                                ...
                                                $pdf->SetXY($x + 30, $y);
                                                $pdf->SetFont('Times','',6);
                                                $data = $row_for_test['test_method_name'].' ('.$smoothness_appearance_tolerance_washing_cycle.' wash)';
...
$pdf->Output('I', "washing_lab_for_trf_id_".$trf_id.".pdf", true);
// ob_end_flush();
?>
