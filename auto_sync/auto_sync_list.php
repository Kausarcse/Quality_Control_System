...
{
	$sql_for_pp = "select DISTINCT * from pp_wise_version_creation_info, process_program_info  where pp_wise_version_creation_info.pp_number = '$pp_number' and pp_wise_version_creation_info.version_id = '$version_number' and pp_wise_version_creation_info.pp_number = process_program_info.pp_number";
	$result_for_pp= mysqli_query($con,$sql_for_pp) or die(mysqli_error($con));
	$row_for_pp = mysqli_fetch_array( $result_for_pp);

	//echo $row_for_pp['customer_name'];

	$option=$option.'<option selected value="'.$row_for_pp['version_name']."?fs?".$row_for_pp['color']."?fs?".$row_for_pp['finish_width_in_inch']."?fs?".$row_for_pp['customer_name']."?fs?".$row_for_pp['version_id']."?fs?".$row_for_pp['customer_id']."?fs?".$row_for_pp['style_name']."?fs?".'"> Version:'.$row_for_pp['version_name'].', Style: '.$row_for_pp['style_name'].', Color: '.$row_for_pp['color'].', Finish Width:'.$row_for_pp['finish_width_in_inch'].' ,Customer:'.$row_for_pp['customer_name'].'</option>';
}
...
<script>
 ...
 function pagination(clicked_id) {
     ...
     if (!search || search.trim() === "" || (search.trim()).length === 0) {
         ...
         $.ajax({
             url: "auto_sync/version_wise_auto_sync_pagination.php",
         ...
         if (!search || search.trim() === "" || (search.trim()).length === 0) {
            //  alert("Empty")
         ...
         else
             {
                 ...
                 $.ajax({
                 url: 'auto_sync/version_wise_auto_sync_search_pagination.php',
                 dataType: 'text',
                 type: 'post',
...
*/	 
 ...
 function adding_specific_row_of_process_adding_table(row_number)
 {
    ...
    table.rows[total_rows].cells[0].innerHTML  = total_rows;
...
function adding_new_form_elements_in_process_adding_table(row_number)
{
	...
	table.rows[new_row_position].cells[2].innerHTML='<input type="text" class="form-control"  id="process_serial_'+new_possible_number_of_process+'" name="process_serial_'+new_possible_number_of_process+'" value="'+new_possible_number_of_process+'">';
...
function Fill_Value_Of_Version_Number_Field_For_Searching(pp_number_for_searching)
{
 ...
 function sending_data_for_edit_serial(get_all_data)
 {
 		$.ajax({
           url: 'auto_sync/edit_auto_sync_serial_list.php',
           dataType: 'text',
           type: 'GET',
           contentType: 'application/x-www-form-urlencoded',
           data: get_all_data,
 ...
 function checking_serial_number(double_value)
 {
 	//alert(double_value);
 	var split_data = double_value.split('?fs?');
	var serial_number = split_data[0];
	var previous_serial_number = split_data[0];
	var total_serial_number = split_data[1];
	var given_serial_number = split_data[2];

	for (var i = 1; i <= total_serial_number; i++) 
	{
		var process_serial_number_target = document.getElementById('process_serial_'+i).value;

		//alert(process_serial_number_target);
		//alert(given_serial_number);
		//alert(i);
		if (previous_serial_number != i) 
		{
			if (process_serial_number_target == given_serial_number) 
			{
				alert("Duplicate Process Serial Number");
				//alert(i);
				document.getElementById('process_serial_'+previous_serial_number).value = previous_serial_number;
			}
		}
...
function searching_data_from_model_db()
{
    ...
    $.ajax({
       url: 'auto_sync/returning_auto_sync_search_result.php',
       dataType: 'text',
       type: 'GET',
       contentType: 'application/x-www-form-urlencoded',
       data: url_encoded_form_data,
       success: function( data, textStatus, jQxhr )
       {
          document.getElementById('div_full_form').innerHTML = data;
...
function sending_edit_serial_auto_sync()
{
	alert('ok');
}
	...
	<div class="panel panel-default"> 
      ...
      <div class="panel-heading" style="color:#191970;font-size:20px;font-weight:bold">
         <div class="row"  data-toggle="collapse" data-target="#search_form_collpapsible_div" onClick="change_up_down_arrow_icon(this.childNodes[5].childNodes[1].id)"> <!-- Row for Panel Heading and Chevron Up Alingment -->
	         <span class="col-sm-11" style="text-align:center;"><b>Search Criteria</b></span>
        ...
        <div class="panel panel-default" id="version_wise_process_info_list">
             ...
             <div  id="pagination" class="form-group form-group-sm" id="form-group_for_test_method_name">
                ...
                <?php
                     ...
                     $sql_for_color = "SELECT * FROM adding_process_to_version_model ORDER BY row_id DESC LIMIT 10";
                     ...
                     <tr>
                         ...
                         <td>
                            <?php
                             ...
                             <button type="button" id="" name="update_pp_info"  class="btn btn-warning btn-xs" onclick="sending_data_for_edit_serial('<?php echo $update_value; ?>')"> Edit Serial </button>
