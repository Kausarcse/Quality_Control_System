...
$select_sql_for_duplicacy="select * from `defining_qc_standard_for_ready_for_mercerize_process` where `pp_number`='$pp_number' and `version_number`='$version_number' and `customer_name`='$customer_name' and `color`='$color' and `finish_width_in_inch`='$finish_width_in_inch' ";
...
else if( mysqli_num_rows($result) < 1)
{
	...
	$insert_sql_statement="INSERT INTO `defining_qc_standard_for_ready_for_mercerize_process`
	                       (
	                       `pp_number`, 
	                       `version_id`, 
	                       `version_number`, 
...
else if($data_insertion_hampering == "No" )
{
	...
	{
		$update_sql_for_pp_monitoringt="UPDATE `pp_monitoring` SET  `current_state`='Waiting for greige issue',`current_status`='Defined Ready For Mercherize standard' 
	...
	{
		$update_sql_for_pp_monitoringt="UPDATE `pp_monitoring` SET  `current_state`='Waiting for defining all standard',`current_status`='Defined Ready For Mercherize standard' 
		WHERE `pp_number`= '$pp_number' and `version_number`='$version_number' and `finish_width_in_inch`='$finish_width_in_inch'";
