...
*/   
		...
		/* $sql = 'select finish_width_in_inch,color,version_name  from `pp_wise_version_creation_info` order by `finish_width_in_inch`';*/
		 $sql = 'select * from `process_program_info`,`pp_wise_version_creation_info` where `process_program_info`.pp_number=`pp_wise_version_creation_info`.pp_number ';
...
*/			  $option=$option.'<option value="'.$row['version_name']."?fs?".$row['color']."?fs?".$row['finish_width_in_inch']."?fs?".$row['customer_name'].'">Version:'.$row['version_name'].', Color: '.$row['color'].', Finish Width:'.$row['finish_width_in_inch'].' ,Customer:'.$row['customer_name'].'</option>';
