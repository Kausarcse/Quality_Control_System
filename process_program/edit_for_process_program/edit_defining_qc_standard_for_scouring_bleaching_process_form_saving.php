...
$select_sql_for_duplicacy="select * from `defining_qc_standard_for_scouring_bleaching_process` where `pp_number`='$pp_number' and
 `version_number`='$version_number' and `customer_name`='$customer_name' and `color`='$color' and `finish_width_in_inch`=$finish_width_in_inch and
  `standard_for_which_process`='$standard_for_which_process' AND `whiteness_min_value`='$whiteness_min_value' AND
...
else if( mysqli_num_rows($result) < 1)
{
	...
	$update_sql_statement="UPDATE  `defining_qc_standard_for_scouring_bleaching_process` SET
