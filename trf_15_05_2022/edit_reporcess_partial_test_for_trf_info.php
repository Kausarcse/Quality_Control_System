...
{  var version_id=document.getElementById('version_id').value;/*
   ...
   var value_for_data= 'process_name_value='+process_name+'?fs?'+version_id;
            ...
            $.ajax({
          url: 'trf/returning_re_process_name_details.php',
          dataType: 'text',
          type: 'post',
          contentType: 'application/x-www-form-urlencoded',
...
function return_machine_name_process_to(process_name)
{  var version_id=document.getElementById('version_id').value;
   ...
   var value_for_data= 'process_name_value='+process_name+'?fs?'+version_id;
              ...
              $.ajax({
                url: 'trf/returning_machine_name_for_reporcess.php',
                dataType: 'text',
                type: 'post',
                contentType: 'application/x-www-form-urlencoded',
                data: value_for_data,
...
function return_trolley_quantity(before_trolley_number_or_batcher_number)
{
  ...
  var version_id=document.getElementById('version_id').value;
  var process_name_from=document.getElementById('process_name_from').value;
 
  /*
   var split_process_name=process_name.split('?fs?');
   var process_id=split_process_name[0];*/
  
  var value_for_trolley_data= 'version_and_process='+version_id+'?fs?'+before_trolley_number_or_batcher_number+'?fs?'+process_name_from;
...
*/
		  	 ...
		  	 $.ajax({
			 		url: 'trf/edit_reprocess_partial_test_for_trf_info_saving.php',
			 		dataType: 'text',
			 		type: 'post',
			 		contentType: 'application/x-www-form-urlencoded',
          ...
          <div class="form-group form-group-sm" id="form-group_for_pp_number">
            ...
            <div class="form-group form-group-sm" id="form-group_for_process_name">
                        <label class="control-label col-sm-3" for="process_name" style="margin-right:0px; color:#00008B;">Process Name: <span style="color:red">*</span> </label>
                            <div class="col-sm-5">
                                <select  class="form-control" id="process_name" name="process_name" onchange="return_machine_name_process_to(this.value)">
                                        <option select="selected" value="<?php echo $row_for_partial_test_for_trf['process_name'].'?fs?'.$row_for_partial_test_for_trf['process_name'] ?>"><?php echo $row_for_partial_test_for_trf['process_name'] ?></option>
                                </select>
   ...
   <div class="panel panel-default">
       ...
       <div class="form-group form-group-sm">
          ...
          <table id="datatable-buttons" class="table table-hover table-bordered">
           ...
           <!--  <button type="submit" id="" name=""  class="btn btn-warning btn-xs" onclick="load_page('trf/edit_partial_test_for_trf_info.php?trf_id=<?php echo $row['trf_id']; ?>')"> Edit</button> -->
