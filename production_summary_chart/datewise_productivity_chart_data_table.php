...
$sql_for_production_date = "SELECT * from datewise_productivity_time WHERE production_date BETWEEN '$from_date' AND '$to_date'";
...
$value_for_planed_change_over_time = array();
...
while($row_for_production_date=mysqli_fetch_array($result_for_production_date))
{
    ...
    if($row_for_production_date['planed_change_over_time'] != 0)
    {
        $value_for_planed_change_over_time[] = $row_for_production_date['planed_change_over_time'];
    }
    else
    {
        $value_for_planed_change_over_time[] = '';
    }
...
$response = array(
    ...
    'value_for_planed_change_over_time' => $value_for_planed_change_over_time,
    'value_for_production_time'  => $value_for_production_time,
    'value_for_production_quantity' => $value_for_production_quantity,
);

//$value = $value_for_change_overtime.'?fs?'.$value_for_breakdown_time.'?fs?'.$value_for_planed_change_over_time.'?fs?'.$value_for_production_time.'?fs?'.$value_for_production_quantity;
