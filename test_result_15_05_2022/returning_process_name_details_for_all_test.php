...
$obj->connection();
  ...
  $sql_for_porcess_selection="SELECT DISTINCT apv.process_serial_no,apv.process_name,apv.process_id,ptti.version_number,ptti.pp_number from partial_test_for_test_result_info ptti
  ...
  $result_for_porcess_selection= mysqli_query($con,$sql_for_porcess_selection) or die(mysqli_error($con));
         ...
         $option='<option value="select" selected="selected">Select  </option>';
		 ...
		 $sql = "SELECT ptfti.process_name
				 ...
				 ,(SELECT after_trolley_number_or_batcher_number from partial_test_for_test_result_info pt 
					where pt.pp_number= '$pp_number'
					and  pt.version_number = '$version_number'
					and pt.process_id = '".$for_current_process_id."') before_trolley_or_batcher_no
