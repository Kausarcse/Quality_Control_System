...
function get_before_trolley_value(before_trolley_number_or_batcher_number)
{
   ...
   get_all_data='?fs?'+version_id+'?fs?'+pp_number+'?fs?'+process_id+'?fs?'+process_name+'?fs?'+style_name+'?fs?'+finish_width_in_inch+"?fs?"+before_trolley_number_or_batcher_number;
  // alert(get_all_data);
   var value_for_data= 'process_name_value='+process_id+'?fs?'+version_id+'?fs?'+pp_number+'?fs?'+before_trolley_number_or_batcher_number+"?fs?";
    ...
    $.ajax({
                  ...
                  {       
                     ...
                     get_all_data='?fs?'+version_id+'?fs?'+pp_number+'?fs?'+process_name+'?fs?'+style_name+'?fs?'+finish_width_in_inch+"?fs?"+before_trolley_number_or_batcher_number+"?fs?"+after_trolley_number_or_batcher_number;
                      // alert(get_all_data);
...
function change_trolley_info(after_trolley_number_or_batcher_number)
{
  ...
  function sending_data_from_database()
 {
    ...
    $.ajax({
			 		...
			 		{
			 				 //alert(data);
               $('#partial_test').load('report/pass_fail_report_for_partial_test.php?all_data='+encodeURIComponent(get_all_data));
			 		},
			 		error: function( jqXhr, textStatus, errorThrown )
			 		{
			 				//console.log( errorThrown );
    ...
    // $('#partial_test').load('report/pass_fail_report_for_partial_test.php?all_data='+all_data);
...
<div id="partial_test">
    ...
    <div class="col-sm-12 col-md-12 col-lg-12" id="panel_load_for_full_form_for_partial">
        <div class="panel panel-default" >
             <div class="panel-heading" style="color:#191970;"><b>Partial Test Report Overview</b></div> <!-- This will create a upper block for FORM (Just Beautification) -->
                        ...
                        <label class="control-label col-sm-3" for="trf_id" style="margin-right:0px; color:#00008B;">TRF No.:</label>
                            <div class="col-sm-5">
                                <select  class="form-control for_auto_complete" id="trf_id" name="trf_id"  onchange="get_all_value_for_trf(this.value)">
                                            ...
                                            <?php 
                                                 $sql = 'select * from `partial_test_for_trf_info` order by partial_test_for_trf_id desc';
                        ...
                        <label class="control-label col-sm-3" for="process_name" style="margin-right:0px; color:#00008B;">Process Name: <span style="color:red">*</span> </label>
                            <div class="col-sm-5">
                                <select  class="form-control" id="process_name" name="process_name" onchange="returning_trolley_info(this.value)">
                                            ...
                                            <?php 
                                                 $sql = 'select process_id, process_name from `process_name`';
                                                 $result= mysqli_query($con,$sql) or die(mysqli_error($con));
                                                 while( $row = mysqli_fetch_array( $result))
                                                 {
