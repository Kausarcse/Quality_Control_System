...
$table="<div id='bleaching_table' style='display:none'><table class='table table-bordered'>
<thead><tr>
<th>Test Name</th>
<th>Test Result</th>
<th>Requirements</th>
 ...
 <tbody> 
  ";
	...
	$sql_for_bleaching_process="select * from defining_qc_standard_for_bleaching_process WHERE customer_name='$customer_name' and pp_number= '$pp_number' and version_number='$version_number'  and `finish_width_in_inch`='$finish_width_in_inch'";
	 ...
	 // $sql_for_bleaching_process="select * from defining_qc_standard_for_bleaching_process WHERE customer_name='Ikea' and pp_number= '5893/2020' and version_number='Qc Back'";

	$result_for_bleaching_process=mysqli_query($con,$sql_for_bleaching_process) or die(mysqli_error($con));
	$row_for_defining_process=mysqli_fetch_array($result_for_bleaching_process);
	...
	// $sql_for_bleaching_process_qc_result="select * from qc_result_for_bleaching_process WHERE customer_name='Ikea' and pp_number= '5893/2020' and version_number='Qc Back'";
	$result_for_bleaching_process_qc=mysqli_query($con,$sql_for_bleaching_process_qc_result) or die(mysqli_error($con));
	$row_for_qc=mysqli_fetch_array($result_for_bleaching_process_qc);
				 ...
				 while( $row = mysqli_fetch_array( $result))
				 {
					 ...
					 if (in_array($row['id'], ['49']))
					 {
					  ...
					  {	
						...
						if($row_for_defining_process['whiteness_min_value']<=$row_for_qc['whiteness_value'] && $row_for_defining_process['whiteness_max_value']>=$row_for_qc['whiteness_value'])
						{
							...
							<td>".$row_for_qc['whiteness_value'].' '.$row_for_defining_process['uom_of_whiteness']."</td>
					 ...
					 if (in_array($row['id'], ['51']))
					 {
					 	...
					 	if ($row_for_defining_process['absorbency_max_value']<>0 && $row_for_qc['absorbency_value']<>0) 
					   {
						...
						if($row_for_defining_process['absorbency_min_value']<=$row_for_qc['absorbency_value'] && $row_for_defining_process['absorbency_max_value']>=$row_for_qc['absorbency_value'])
						{
							...
							$table.="<tr>
							<td>".$row['test_name_method']."</td>
							<td>".$row_for_qc['absorbency_value'].' '.$row_for_defining_process['uom_of_absorbency']."</td>
							<td>".$row_for_defining_process['absorbency_min_value'].' to '.$row_for_defining_process['absorbency_max_value']."</td>
							<td>Pass</td>
							</tr>";
						}
						else {
					  ...
					  if ($row_for_defining_process['residual_sizing_material_max_value']<>0 && $row_for_qc['residual_sizing_material_value']<>0) 
					   {
                         ...
                         if($row_for_defining_process['residual_sizing_material_min_value']<=$row_for_qc['residual_sizing_material_value'] && $row_for_defining_process['residual_sizing_material_max_value']>=$row_for_qc['residual_sizing_material_value'])
						{
							...
							$table.="<tr>
							<td>".$row['test_name_method']."</td>
							<td>".$row_for_qc['residual_sizing_material_value'].' '.$row_for_defining_process['uom_of_residual_sizing_material']."</td>
					  ...
					  if (in_array($row['id'], ['6']))
					 {
						...
						if($customer_type == 'american')
						{
							...
							$surface_fuzzing_and_pilling_value = $row_for_qc['resistance_to_surface_fuzzing_and_pilling_value'];
						...
						if($customer_type == 'european')
						{
							...
							if($row_for_defining_process['surface_fuzzing_and_pilling_min_value']<=$row_for_qc['resistance_to_surface_fuzzing_and_pilling_value'] && $row_for_defining_process['surface_fuzzing_and_pilling_max_value']>=$row_for_qc['resistance_to_surface_fuzzing_and_pilling_value'])
							{
								...
								$table.="<tr>
										<td>".$row['test_name_method']."</td>
										<td>".$resistance_to_surface_fuzzing_and_pilling_value.' '.$row_for_defining_process['uom_of_resistance_to_surface_fuzzing_and_pilling']."</td>
										
										<td>".$row_for_defining_process['surface_fuzzing_and_pilling_tolerance_range_math_operator'].' '.$surface_fuzzing_and_pilling_tolerance_value.' '.$row_for_defining_process['uom_of_resistance_to_surface_fuzzing_and_pilling']."</td>
										<td>Pass</td>
										</tr>";
							}
							else {
