...
$table="<div id='ready_for_print_table' style='display:none'><table class='table table-bordered'>
<thead><tr>
<th>Test Name</th>
<th>Test Result</th>
<th>Requirements</th>
<th>Remarks</th>
 ...
 <tbody> 
  ";
	...
	$sql_for_ready_for_printing="select * from defining_qc_standard_for_ready_for_printing_process WHERE customer_name='$customer_name' and pp_number= '$pp_number' and version_number='$version_number'  and `finish_width_in_inch`='$finish_width_in_inch'";

	$result_for_ready_for_printing=mysqli_query($con,$sql_for_ready_for_printing) or die(mysqli_error($con));
	$row_for_defining_process=mysqli_fetch_array($result_for_ready_for_printing);

	/***************** Displaying Result from qc_standard table [END] *****************/


	/************ Displaying Result from qc_result table [Start] ************/
	$sql_for_ready_for_printing_qc_result="select * from qc_result_for_ready_for_printing_process WHERE customer_name='$customer_name' and pp_number= '$pp_number' and version_number='$version_number' and `finish_width_in_inch`='$finish_width_in_inch' and `before_trolley_number_or_batcher_number`='$before_trolley_number_or_batcher_number' and `after_trolley_number_or_batcher_number`='$after_trolley_number_or_batcher_number'";
    
    
	$result_for_ready_for_printing_qc=mysqli_query($con,$sql_for_ready_for_printing_qc_result) or die(mysqli_error($con));
	$row_for_qc=mysqli_fetch_array($result_for_ready_for_printing_qc);
										...
										if($row_for_defining_process['whiteness_min_value']<=$row_for_qc['whiteness_left_value'] && $row_for_defining_process['whiteness_max_value']>=$row_for_qc['whiteness_left_value'] && $row_for_defining_process['whiteness_min_value']<=$row_for_qc['whiteness_center_value'] && $row_for_defining_process['whiteness_max_value']>=$row_for_qc['whiteness_center_value'] && $row_for_defining_process['whiteness_min_value']<=$row_for_qc['whiteness_right_value'] && $row_for_defining_process['whiteness_max_value']>=$row_for_qc['whiteness_right_value'])
											{
												...
												<td>Left:".$row_for_qc['whiteness_left_value'].' Middle:'.$row_for_qc['whiteness_center_value'].' Center:'.$row_for_qc['whiteness_right_value'].' '.$row_for_defining_process['uom_of_whiteness']."</td>	
