...
$obj->connection();
  $version_and_process=$_POST['process_name_value'];

  $splitted_data=explode('?fs?',$version_and_process);
  $process_id=$splitted_data[0]; 
  $version_id=$splitted_data[1]; 
  $before_trolley_number_or_batcher_number=$splitted_data[3]; 
  ...
  $sql_for_porcess_selection="SELECT DISTINCT apv.process_serial_no,apv.process_name,apv.process_id,ptti.version_number,ptti.pp_number,ptti.design,ptti.style,ptti.finish_width_in_inch,ptti.before_trolley_number_or_batcher_number from partial_test_for_test_result_info ptti
  ...
  $result_for_porcess_selection= mysqli_query($con,$sql_for_porcess_selection) or die(mysqli_error($con));
         ...
         $option='<option select"selected" value="">Select After Trolley</option>';
		...
		/* $sql = "SELECT ptfti.process_name
		 ...
		 $sql = "SELECT ptfti.after_trolley_number_or_batcher_number
		          FROM partial_test_for_test_result_info ptfti
				WHERE ptfti.process_id='".$for_current_process_id."' AND ptfti.pp_number='$pp_number' AND ptfti.version_number='$version_number'
				and  ptfti.style = '$style'
				and  ptfti.design = '$design'
				and  ptfti.finish_width_in_inch = '$finish_width_in_inch'
				and  ptfti.before_trolley_number_or_batcher_number = '$before_trolley_number_or_batcher_number'";
		 ...
		 {    
		  ...
		  $option=$row['after_trolley_number_or_batcher_number'];
