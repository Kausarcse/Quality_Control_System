...
// $ready_for_ready_for_raising_process_quality=$_POST['ready_for_ready_for_raising_process_quality'];
...
$select_sql_for_duplicacy="select * from `qc_result_for_ready_for_raising_process`
...
{
	$select_sql_max_primary_key="select MAX(max_report_serial_no) as max_report_serial_no FROM (select CONVERT(substring(report_serial_no,5,LENGTH(report_serial_no)),UNSIGNED) as max_report_serial_no from qc_result_for_ready_for_raising_process) as temp_qc_result_for_ready_for_raising_process_table"; //converted into string and find max
    ...
    if($row_for_max_primary_key['max_report_serial_no']==0)
    {
    	...
    	$current_report_serial_no='RRR_1';

    }
    else
    {

    	$current_report_serial_no ="RRR_".($row_for_max_primary_key['max_report_serial_no']+1);

    }

	$insert_sql_statement="insert into `qc_result_for_ready_for_raising_process` 
	          (
	           `pp_number`,
               `version_number`,
			  `customer_name`,
	...
	else
	{
		...
		$sql_for_monitoring_ready_for_raising = "SELECT * FROM pp_monitoring WHERE `pp_number`= '$pp_number' and `version_number`='$version_number' and `finish_width_in_inch`='$finish_width_in_inch' ";
		$result_for_monitoring_ready_for_raising= mysqli_query($con, $sql_for_monitoring_ready_for_raising);
		$row_for_monitoring_ready_for_raising = mysqli_fetch_assoc($result_for_monitoring_ready_for_raising);
		$current_status_monitoring_san = $row_for_monitoring_ready_for_raising['current_status'];
		...
		{
			$sql_for_process = "SELECT * FROM qc_result_for_ready_for_raising_process WHERE pp_number = '$pp_number' AND version_number = '$version_number' and finish_width_in_inch = '$finish_width_in_inch'
