...
// $sql_qc_printing="select id from qc_result_for_printing_process where pp_number = '$trf_pp_number'";
// $res_sql_qc_printing=mysqli_query($con,$sql_qc_printing) or die(mysqli_error($con));
// $row_for_id=mysqli_fetch_assoc($res_sql_qc_printing);

// $printing_id = $row_for_id['id'];


$sql_for_printing="select * from qc_result_for_printing_process where pp_number = '$trf_pp_number' and 
...
$row_for_printing = mysqli_fetch_array( $result_for_printing);

?>



<script type='text/javascript' src='process_qc_result/qc_result_for_printing_process_form_validation.js'></script>
...
{   
	var db_table_name="defining_qc_standard_for_printing_process";
...
*/  
 ...
 function sending_data_of_qc_result_for_printing_process_form_for_saving_in_database()
 {


       var validate = Printing_Form_Validation();
       var url_encoded_form_data = $("#qc_result_for_printing_process_form").serialize(); //This will read all control elements value of the form	
       if(validate != false)
	   {


		  	 $.ajax({
			 		url: 'process_qc_result/edit_qc_result_for_printing_process_saving.php',
			 		dataType: 'text',
			 		type: 'post',
			 		contentType: 'application/x-www-form-urlencoded',
 ...
 }//End of function sending_data_of_qc_result_for_printing_process_form_for_saving_in_database()
...
<div class="col-sm-12 col-md-12 col-lg-12">
		...
		<div class="panel panel-default"> <!-- This div will create a block/panel for FORM -->
			...
			<body id="target">
				...
				<div class="panel-heading" style="color:#191970;"><b>Qc Result For Printing Process / <a href="#">Edit</a></b></div> <!-- This will create a upper block for FORM (Just Beautification) -->

				<form class="form-horizontal" action="" style="margin-top:10px;" name="qc_result_for_printing_process_form" id="qc_result_for_printing_process_form">
				...
				<div class="form-group form-group-sm" id="form-group_for_pp_number" onchange="Fill_Value_Of_Version_Number_Field(this.value)">
					...
					<!-- 	<div class="form-group form-group-sm" id="form-group_for_total_short_or_excess_in_percentage">
						...
						<div class="form-group form-group-sm" id="form-group_for_machine_name">
						 ...
						 <div class="form-group form-group-sm" id="form-group_for_rubbing_value_in_fabric">

			              <label class="control-label col-sm-3" for="rubbing" style="color:#00008B; margin-top:23px;">Cf To Rubbing :</label>
			              
			              <div class="col-sm-3 text-center">
			                <label for="rubbing_value" style="font-size:12px; color:#000066;"> Dry</label>
			                <input type="text" class="form-control" id="cf_to_rubbing_dry_value" name="cf_to_rubbing_dry_value" value="<?php echo $row_for_printing['cf_to_rubbing_dry_value']?>" style="padding-right:4px;" required>
			              </div>
			              
			              
			              <div class="col-sm-3 text-center">
			                <label for="rubbing_value" style="font-size:12px; color:#000066;"> Wet</label>
			                <input type="text" class="form-control" id="cf_to_rubbing_wet_value" name="cf_to_rubbing_wet_value" value="<?php echo $row_for_printing['cf_to_rubbing_wet_value']?>" style="padding-right:4px;" required>
			              </div>
                         

                         <input type="hidden" class="form-control" id="uom_of_cf_to_rubbing_dry" name="uom_of_cf_to_rubbing_dry" placeholder="%" required>
			             <input type="hidden" class="form-control" id="uom_of_cf_to_rubbing_wet" name="uom_of_cf_to_rubbing_wet" placeholder="%" required>



			          </div>  <!-- End <div class="form-group form-group-sm" id="form-group_for_rubbing_value"> -->
						...
						<label class="control-label col-sm-3" for="status" style="margin-right:15px;color:#00008B;">Status:</label>
								<input type="radio" class="form-check-input"  value="OK" id="status" name="status" <?php if($row_for_printing['status']=="OK"){ echo "checked";}?>>
								<label class="form-check-label control-label" for="status" style="margin-right:15px;">OK</label>
								<input type="radio" class="form-check-input"  value="NOT OK" id="status" name="status" <?php if($row_for_printing['status']=="NOT OK"){ echo "checked";}?>>
						...
						<div class="form-group form-group-sm" id="form-group_for_remarks">
								...
								<div class="col-sm-6">
									<textarea class='form-control' id='remarks' name='remarks' rows='5' value="<?php echo $row_for_printing['remarks']?>"><?php echo $row_for_printing['remarks']?></textarea>
								</div>
						...
						<div class="form-group form-group-sm" id="form-group_for_current_state">
						   <label class="control-label col-sm-3" for="current_state" style="margin-right:15px;color:#00008B;">NOTE(IS IT A LAST PROCESS):</label>
								<input type="radio" class="form-check-input"  value="PP Completed" id="current_state" name="current_state" <?php if($row_for_printing['current_state']=="PP Completed"){ echo "checked";}?>>
								<label class="form-check-label control-label" for="current_state" style="margin-right:15px;">Yes</label>
								<input type="radio" class="form-check-input"  value="PP in progress" id="current_state" name="current_state" <?php if($row_for_printing['current_state']=="PP in progress"){ echo "checked";}?>>
						...
						<div class="form-group form-group-sm">
								<div class="col-sm-offset-3 col-sm-6">
									<button type="button" class="btn btn-primary" onClick="sending_data_of_qc_result_for_printing_process_form_for_saving_in_database()">Save</button>
