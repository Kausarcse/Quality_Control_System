...
$select_sql_for_duplicacy="select * from `qc_result_for_ready_for_mercerize_process` where `pp_number`='$pp_number' and `version_number`='$version_number' and `customer_name`='$customer_name' and `color`='$color' and `finish_width_in_inch`=$finish_width_in_inch and `standard_for_which_process`='$standard_for_which_process' and   
...
else if(mysqli_num_rows($result) < 1)
{
	...
	$select_sql_max_primary_key="select MAX(max_report_serial_no) as max_report_serial_no FROM (select CONVERT(substring(report_serial_no,5,LENGTH(report_serial_no)),UNSIGNED) as max_report_serial_no from qc_result_for_ready_for_mercerize_process) as temp_qc_result_for_ready_for_mercerize_process_table"; //converted into string and find max
    ...
    if($row_for_max_primary_key['max_report_serial_no']==0)
    {
    	...
    	$current_report_serial_no='RMR_1';

    }
    else
    {

    	$current_report_serial_no ="RMR_".($row_for_max_primary_key['max_report_serial_no']+1);

    }

	$insert_sql_statement="insert into `qc_result_for_ready_for_mercerize_process` ( `pp_number`,`version_number`,`customer_name`,`color`,`finish_width_in_inch`,`standard_for_which_process`,`date`,
	...
	`whiteness_value_in_left_side_of_fabric`,`whiteness_value_in_center_of_fabric`,`whiteness_value_in_right_side_of_fabric`,`uom_of_whiteness`,`bowing_and_skew_value`,`uom_of_bowing_and_skew`,
	`Ph_value_in_left_side_of_fabric`,`Ph_value_in_center_of_fabric`,`Ph_value_in_right_of_fabric`,`uom_of_ph`,`status`,`remarks`,`recording_person_id`,`recording_person_name`,`recording_time`,`version_id`,
	...
	'$Ph_value_in_center_of_fabric',
	'$Ph_value_in_right_of_fabric',
	'$uom_of_ph',
	'$status',
	'$remarks',
	'$user_id',
	...
	else
	{
		...
		{
			$update_sql_for_pp_monitoring="UPDATE `pp_monitoring` SET  `current_status`='Re-Ready For Mercerize Process',`current_state`='$current_state' WHERE `pp_number`= '$pp_number' and `version_number`='$version_number' and `finish_width_in_inch`='$finish_width_in_inch'";
		...
		{
			$update_sql_for_pp_monitoring="UPDATE `pp_monitoring` SET  `current_status`='2nd-Re-Ready For Mercerize Process',`current_state`='$current_state' WHERE `pp_number`= '$pp_number' and `version_number`='$version_number' and `finish_width_in_inch`='$finish_width_in_inch'";

			mysqli_query($con,$update_sql_for_pp_monitoring) or die(mysqli_error($con));
		}
		else if($current_status_monitoring_san=='2nd-Re-Ready For Mercerize Process')
		{
			$update_sql_for_pp_monitoring="UPDATE `pp_monitoring` SET  `current_status`='3rd-Re-Ready For Mercerize Process',`current_state`='$current_state' WHERE `pp_number`= '$pp_number' and `version_number`='$version_number' and `finish_width_in_inch`='$finish_width_in_inch'";

			mysqli_query($con,$update_sql_for_pp_monitoring) or die(mysqli_error($con));
		}
		else if($current_status_monitoring_san=='3rd-Re-Ready For Mercerize Process')
		{
			$update_sql_for_pp_monitoring="UPDATE `pp_monitoring` SET  `current_status`='4th-Re-Ready For Mercerize Process',`current_state`='$current_state' WHERE `pp_number`= '$pp_number' and `version_number`='$version_number' and `finish_width_in_inch`='$finish_width_in_inch'";

			mysqli_query($con,$update_sql_for_pp_monitoring) or die(mysqli_error($con));
		}
		else
		{
			$sql_for_process = "SELECT * FROM qc_result_for_ready_for_mercerize_process WHERE pp_number = '$pp_number' AND version_number = '$version_number' and finish_width_in_inch = '$finish_width_in_inch'
			...
			{
				$update_sql_for_pp_monitoring="UPDATE `pp_monitoring` SET  `current_status`='Ready For Mercerize Process',`current_state`='$current_state' WHERE `pp_number`= '$pp_number' and `version_number`='$version_number' and `finish_width_in_inch`='$finish_width_in_inch'";
